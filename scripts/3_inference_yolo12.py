...
def monitor_crowd(source, model_path, output_dir, threshold=9, conf_threshold=0.5):
    """
    Runs YOLO12 inference on a source (image, video, or webcam),
    counts the number of people, detects overcrowding, and saves the output.
    
    Args:
        source: Path to image/video or '0' for webcam.
        model_path: Path to the trained YOLO12 weights (e.g., best.pt).
    ...
    try:
        ...
        print(f"Details: {e}")
        print("\nIf you are trying to use a newly released model like YOLO12, your 'ultralytics' library might not support it yet, or the weights file couldn't be automatically downloaded.")
        print("Please ensure:")
    ...
    while cap.isOpened():
        ...
        # Run YOLO12 inference on the frame
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="yolo12m.pt", help="Path to trained model weights")
    ...
    if not os.path.exists(model_path):
        # Fallback to local execution using default yolo12 downloads
