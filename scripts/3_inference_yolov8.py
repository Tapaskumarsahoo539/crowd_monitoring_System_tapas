...
def monitor_crowd(source, model_path, output_dir, threshold=9, conf_threshold=0.5):
    """
    Runs YOLOv8 inference on a source (image, video, or webcam),
    counts the number of people, detects overcrowding, and saves the output.
    
    Args:
        source: Path to image/video or '0' for webcam.
        model_path: Path to the trained YOLOv8 weights (e.g., best.pt).
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="yolov8m.pt", help="Path to trained model weights")
    ...
    if not os.path.exists(model_path):
        # Fallback to local execution using default yolov8 downloads
